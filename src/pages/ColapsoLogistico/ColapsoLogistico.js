...
const Simulador = () => {
  ...
  const onConnected = () => {
    stompClient.subscribe("/collapse", onSimulation);
    stompClient.subscribe("/collapse/response", onSimulationResponse);
    sendDate();
  };
  ...
  const sendDate = () => {
    ...
    stompClient.send("/app/collapse", {}, JSON.stringify(date));
  };
  ...
  const simulacionData = (
    <>
      <div className="purpleBox opdia-envio-title">Colapso logístico</div>
      <div className="blackBox opdia-envio-container p10" style={{ height: "100%", overflow: "auto" }}>
        ...
        <div className="row mb-3">
          ...
          >
            <RedirectButton text="Iniciar simulación colapso" icon={<ArrowForwardIosIcon/>}/>
          </span>}
          <span
            className="w-100"
            onClick={() => {
